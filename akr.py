...
import threading
stopuser = {}
token = '6970471002:AAF2TGHE8eGOHnka3prm79SAMLTwRJ25lko'
	...
	def my_function():
	  ...
	  "timer": "none",
		...
		if BL == '𝗙𝗥𝗘𝗘':	
			...
			contact_button = types.InlineKeyboardButton(text="✨ 𝗢𝗪𝗡𝗘𝗥  ✨", url="https://t.me/AKRSPAM")
...
𝒀𝑶𝑼𝑹 𝑷𝑳𝑨𝑵 𝑵𝑶𝑾 {BL}</b>
	''',reply_markup=keyboard)
			return
		keyboard = types.InlineKeyboardMarkup()
		contact_button = types.InlineKeyboardButton(text="✨ 𝗝𝗢𝗜𝗡 ✨", url="https://t.me/AKRSPAM")
		keyboard.add(contact_button)
...
def main(message):
	  ...
	  "timer": "none",
			...
			with open('data.json', 'w') as json_file:
				json.dump(existing_data, json_file, ensure_ascii=False, indent=4)	
			keyboard = types.InlineKeyboardMarkup()
			contact_button = types.InlineKeyboardButton(text="✨ 𝗢𝗪𝗡𝗘𝗥  ✨", url="https://t.me/NN_US")
			keyboard.add(contact_button)
...
𝘽𝙤𝙩 𝘽𝙮 AKRSPAM''', reply_markup=mes)
...
def start(message):
	def my_function():
		...
		try:
			...
			pas ='AKR-'+''.join(random.choices(characters, k=4))+'-'+''.join(random.choices(characters, k=4))+'-'+''.join(random.choices(characters, k=4))
			current_time = datetime.now()
